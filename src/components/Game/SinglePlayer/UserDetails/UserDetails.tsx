...
function UserDetails({accentPrimary,accentSecondary,gameName}:UserDetailsProp){
    ...
    return (
        <div className={gameStartAnimate?'user-details user-active':'user-details'}>
            <section className="user-section" style={{background:accentPrimary}}>
                <div className="user-section-header">
                    <h1>You Selected:</h1>
                    <h2>{gameName}</h2>
                </div>
            </section>
            <section className="user-section" style={{background:accentSecondary}}>
                <div className="user-details-form">
                    <h2>Please Enter Your Name Before Starting Game</h2>
                    <form className='details' action="">
                        <input type="text" name="username" id="" ref={inputRef}  required maxLength={20}/>
                        <button style={{background:accentPrimary}}  onClick={(e)=>{startGamePlay(e)}}>Play Game!</button>
                    </form>
